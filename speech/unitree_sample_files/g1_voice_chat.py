...
def speak_g1_text(
    ...
    """Generate and play one complete prompt through G1 on the main thread."""
    with tempfile.TemporaryDirectory(prefix="g1-prompt-") as directory:
        ...
        if not ok or sample_rate != 16000 or channels != 1:
            raise RuntimeError("Invalid G1 prompt WAV generated by Edge TTS")
...
def execute_face_wave_with_speech(
    ...
    """Run face wave and greeting sequentially on the main DDS thread."""
    with tempfile.TemporaryDirectory(prefix="g1-wave-greeting-") as directory:
        ...
        code = arm_action_client.ExecuteAction(action_map["face wave"])
        if code != 0:
            raise RuntimeError(f"G1 face-wave action failed with SDK code {code}")

        try:
            # Avoid overlapping arm and audio RPC calls. The action runs on G1
            # after this RPC returns; a short delay lets DDS settle before the
            # larger binary PlayStream request.
            time.sleep(args.face_wave_audio_delay_seconds)
            play_g1_interruptibly(
                audio_client,
                pcm,
                threading.Event(),
            ...
            # Release only after complete audio playback. If the built-in face
            # wave animation finishes earlier, speech continues unaffected.
            code = arm_action_client.ExecuteAction(action_map["release arm"])
            if code != 0:
                raise RuntimeError(f"G1 arm release failed with SDK code {code}")
        time.sleep(args.action_settle_seconds)
        print("Action: face wave greeting complete; normal arms restored.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--face-wave-audio-delay-seconds",
        type=float,
        default=0.5,
        help="DDS settling delay between face-wave RPC and greeting PlayStream",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    try:
        while True:
            ...
            print(f"Robot prompt: {DRAW_OMIKUJI_TEXT}")
            try:
                speak_g1_text(
                    client,
                    DRAW_OMIKUJI_TEXT,
                ...
                # Continue to the camera-specific prompt even if this optional
                # acknowledgement cannot be played.
                print(f"Omikuji draw prompt failed: {error}", file=sys.stderr)
