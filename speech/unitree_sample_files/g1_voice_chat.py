...
def execute_face_wave_with_speech(
    audio_client: AudioClient,
    arm_action_client: G1ArmActionClient,
    text: str,
    args: argparse.Namespace,
) -> None:
    """Start face wave and prepared G1 speech together, then await both."""
    with tempfile.TemporaryDirectory(prefix="g1-wave-greeting-") as directory:
        wav_path = Path(directory) / "greeting.wav"
        # Prepare remote Edge TTS before moving, so speech and motion start
        # together rather than making the arm wait on network synthesis.
        make_g1_wav(text, wav_path)
        pcm, sample_rate, channels, ok = read_wav(str(wav_path))
        if not ok or sample_rate != 16000 or channels != 1:
            raise RuntimeError("Invalid face-wave greeting WAV generated by Edge TTS")

        speech_result: queue.Queue = queue.Queue(maxsize=1)
        ...
        def play_speech() -> None:
            try:
                ...
                speech_result.put(None)
            except BaseException as error:
                speech_result.put(error)

        speech_thread = threading.Thread(
            target=play_speech,
            name="g1-wave-greeting-audio",
            daemon=True,
        )
        print(f"Robot greeting: {text}")
        speech_thread.start()
        try:
            execute_face_wave(
                arm_action_client,
            ...
            # Do not stop audio when the face wave ends. Wait for the complete
            # sentence and its playback tail before starting the next pose.
            speech_thread.join()

        speech_error = speech_result.get()
        if speech_error is not None:
            raise RuntimeError(
                f"G1 face-wave greeting playback failed: {speech_error}"
            ) from speech_error
